...
def main():
    ...
    f = open('input.txt', 'r')
    santapos = [0, 0]    # x, y coordinates in a 2-dimensional grid
    robopos = [0, 0]    # x, y coordinates in a 2-dimensional grid
    santamap = {}
    robomap = {}
    i = 0
    ...
    for char in string:
        ...
        if i % 2 != 0:
            house = str(santapos[0]) + ',' + str(santapos[1]) # Can't store a list as a dictionary key, so convert it to a string

            if santamap.has_key(house):
                santamap[house] += 1
            else:
                santamap[house] = 1

            print 'SANTA\tStep {}: {} {} \t House {} : Count {}'.format(i, char, santapos, house, santamap[house]) # Debugging output

        else:
            house = str(robopos[0]) + ',' + str(robopos[1]) # Can't store a list as a dictionary key, so convert it to a string

            if robomap.has_key(house):
                robomap[house] += 1
            else:
                robomap[house] = 1

            print '\tROBO\tStep {}: {} {} \t House {} : Count {}'.format(i, char, robopos, house, robomap[house]) # Debugging output


    map = dict(santamap.items() + robomap.items())
    # print santamap.items() # Debugging output
    # print robomap.items() # Debugging output
    # print 'There were {} houses that received at least one present'.format(len(map))
    print ('Santa delivered to {} houses, while Robosanta delivered to {} houses for a total of {} houses').format(len(santamap), len(robomap), len(map))
